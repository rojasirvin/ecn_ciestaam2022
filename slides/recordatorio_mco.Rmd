...
Sustituyendo lo que es $y_i$


$$
\begin{align}
\hat{\beta}_{MCO}&=\sum_i w_i (\hat{\alpha}+\hat{\beta}x_i+u_i) = \hat{\alpha}\sum_i w_i + \hat{\beta}\sum_i w_i x_i + \sum_i w_i u_i \\
&= \hat{\beta}+\sum_i w_i u_i
\end{align}
$$
...
Notemos primero que los pesos $\sum_i w_i$ suman 1:
...
$$
\begin{align}
E(\hat{\beta})&=E(\beta+ \sum_i w_i u_i) \\
...
La propiedad de insesgadez dice que si tuviéramos muchas muestras, **todas de tamaño $N$**, y con cada una obtuviéramos $\hat{\beta}_2$ para cada muestra, $\hat{\beta}$ estará centrado alrededor de $\beta$
...
.pull-right[
```{r echo=F, message=FALSE, warnings=FALSE, results=T, cache=T}
